...
const mapStateToProps = (state, ownProps) => {
    ...
    const active = state.selectedCat;
...
export default connect(mapStateToProps, {addFavorite, removeFavorite})(class IngredientListItem extends Component {
    ...
    componentWillReceiveProps(nextProps){
        ...
        if(this.state.searchText !== nextProps.searchText && nextProps.searchText.length > 1){
    ...
    render() {
        ...
        const leftContent = [<TouchableOpacity onPress={() => this.toggleExpand()} style={{
            ...
            style={{alignSelf: 'flex-end', height: 46, width: 68, alignItems: 'center', justifyContent: 'center'}}>
        ...
        return (
            <Swipeable onRef={ref => this.swipeable = ref} leftActionActivationDistance={68} rightActionActivationDistance={!isFavorite ? 0 : 68} leftButtonWidth={68}
                    ...
                    flex: 1,
                    height: 46,
                }, s.paddingSmall, {paddingLeft: 0}]}><View style={[{flex: 0.9, height: 46,     flexDirection: 'row', alignItems: 'center'}]} >
                ...
                </View><Text style={[s.h2, this.props.active ? s.normal :'']} numberOfLines={1}><Highlighter
                highlightStyle={[s.bold, {marginHorizontal: 5}]}
                searchWords={[this.props.active]}
                textToHighlight={title}
            /></Text></View><Text><IngridIcon
                    icon="ArrowSmallRight"
                    fontSize={14}/></Text></TouchableOpacity></Swipeable>
